...
| melonDS | <https://github.com/melonDS-emu/melonDS> at `906e9ebb27da8c6a715cd7abab4abfe8a8d29427` (tree `60c6724f8695bdbe5e21c4367b18293f7f811fc2`) | GPL-3.0-or-later; see `third_party/melonDS/LICENSE` and the license declaration in `third_party/melonDS/README.md` | DualBoy builds upstream's static `core` with the desktop frontend, GDB stub, OpenGL renderer, JIT, release LTO, and embedded build metadata disabled. Because upstream does not put `src/net/LocalMP.cpp` in `core`, that exact untouched source is compiled into a separate internal archive. All platform and pair adaptation is under `src/engines/melonds/`; the submodule is unmodified. |
...
3. For melonDS, verify the proposed commit and tree, review its root license plus
   ...
   `LocalMP.cpp` is part of that target.
